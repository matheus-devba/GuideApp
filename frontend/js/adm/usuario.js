...
async function renderUsuario(id) {
    try {
        ...
        const usuarioData = await response.json();// 2. Aguarda a conversão da resposta para um Objeto/Array JavaScript

        const container = document.querySelector(".usuario-form")
        if(!container) return

        const innerHTMLUsaurio = insertDataUsaurio(usuarioData)
        container.innerHTML = innerHTMLUsaurio


    } catch (error) {
        console.error("Erro ao busca usuario do servidor:", error);
    }
...
async function insertDataUsuario(data) {
    ...
    return `
        <div class="form-group">
            ...
            <input type="text" id="store-name" name="store-name" value="${nomeDaLoja}">
        </div>

        <div class="form-group">
            <label>Nome do Usuario</label>
            <input type="text" id="nome-usuario" name="nome-usuario" value="${nomeDaLoja}">
        </div>
