...
export async function initHome() {
    ...
    await renderPainel(containerEl)
}
...
async function renderPainel(containerEl) {
  const eventos = await requestJSON(`${API_BASE_URL}/api/eventos`);
  const categorias = await requestJSON(`${API_BASE_URL}/api/categorias`)
  const lojas = await requestJSON(`${API_BASE_URL}/api/lojas`)
  const produtosAll = await requestJSON(`${API_BASE_URL}/api/produtos/ativosAll`)

  const lojasAtivas = lojas.map((loja) => loja.id).length
  const categoriasAtivas = categorias.map((cat) => cat.id).length
  const produtosCadastrados = produtosAll.map((prod) => prod.id).length
  ...
  containerEl.innerHTML = `
    <div class="painel">
      ...
      <section class="painel-secao painel-kpis">
        <h2 class="painel-titulo">Saúde</h2>
        <div class="kpi-grid">
          ${kpiCard(lojasAtivas, 'Lojas ativas')}
          ${kpiCard(produtosCadastrados, 'Produtos cadastrados')}
          ${kpiCard(categorias.length, 'Categorias')}
        </div>
      </section>

      <section class="painel-secao painel-kpis">
        <h2 class="painel-titulo">Desempenho das lojas</h2>
        <div class="kpi-grid">
      ...
      <section class="painel-secao">
        <h2 class="painel-titulo">De onde vieram os visitantes?</h2>
